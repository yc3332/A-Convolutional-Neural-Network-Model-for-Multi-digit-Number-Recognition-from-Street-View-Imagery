...
class batch_setup(object):
    ...
    def _read_and_decode(filename_queue):
        ...
        image = batch_setup._preprocess(tf.decode_raw(features['image'], tf.uint8))
        length = tf.cast(features['length'], tf.int32)
        digits = tf.cast(features['digits'], tf.int32)
        return image, length, digits

    @staticmethod
    def my_batch(path_to_tfrecords_file, num_examples, batch_size, shuffled):
        # create batches
        ...
        image, length, digits = batch_setup._read_and_decode(filename_queue)
